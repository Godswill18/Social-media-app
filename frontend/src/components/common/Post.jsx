...
const Post = ({post}) => {
	...
	const {mutate:deletePost, isPending:isDeleting} = useMutation({
		...
		onSuccess: () => {
			...
			queryClient.invalidateQueries({ queryKey: ["posts"] }); // Invalidate the posts query to refetch posts
		}
	})
	...
	const {mutate:likePost, isPending:isLiking} = useMutation({
		...
		onSuccess: (updatedLikes) => {
			...
			// Optimistically update the post likes in the cache
	...
	const {mutate:commentPost, isPending:isCommenting} = useMutation({
		...
		onSuccess: (updatedComments) => {
			...
			// Optimistically update the post comments in the cache
			// queryClient.setQueryData(["posts"], (oldData) => {
			// 	return oldData.map((p) => {
			// 		if (p._id === post._id) {
	...
	const handlePostComment = (e) => {
		e.preventDefault();
		if (isCommenting) return; // Prevent multiple clicks and empty comments
		commentPost();
	};
